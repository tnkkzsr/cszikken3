...
def p_func_decl(p):
    ...
    varscope = Scope.GLOBAL_VAR
    retval = getRegister()
    addCode(LLVMCodeLoad(retval,Operand(OType.NAMED_REG, name=p[2])))
...
def p_func_name(p):
    ...
    func_name : IDENT
    '''
    symtable.insert(p[1], Scope.FUNC)
...
def p_func_act2(p):
    ...
    func_act2 :
    '''
    fundefs.append(Fundef(p[-2], 'i32'))
    global varscope
    varscope = Scope.LOCAL_VAR
    addCode(LLVMCodeAlloca(Operand(OType.NAMED_REG, name=p[-2])))
    
def p_act_func_args_set(p):
    '''
    act_func_args_set :
    '''
    global varscope
...
def p_act_func_args_done(p):
    '''
    act_func_args_done :
    '''   
    fundefs.append(Fundef(p[-4], 'i32', p[-1]))
    global varscope
    varscope = Scope.LOCAL_VAR
...
def p_func_call(p):
    ...
    retval = getRegister()
    if len(p) == 4:
        addCode(LLVMCodeCallFunc(p[1],retval,'i32'))
    else:
        addCode(LLVMCodeCallFunc(p[1],'i32', p[3]))
